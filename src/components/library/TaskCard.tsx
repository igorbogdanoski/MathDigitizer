...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  return (
    <Card 
      ...
      onDrop={onDrop}
      className={`overflow-hidden border-slate-200 transition-all duration-300 cursor-pointer rounded-2xl ${isSelected ? 'ring-2 ring-indigo-500 border-indigo-500 shadow-lg shadow-indigo-900/10' : 'hover:shadow-lg hover:border-indigo-300 shadow-sm'} ${isDeleting ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}
      onClick={onSelect}
    >
      <div className="bg-slate-50/80 dark:bg-slate-800/80 px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-start backdrop-blur-sm">
        <div className="flex gap-4 items-start">
          <div className="mt-0.5 cursor-grab active:cursor-grabbing text-slate-300 hover:text-slate-500 dark:hover:text-slate-400 transition-colors">
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {task.type === 'theory' && (
                <span className="text-[9px] font-medium uppercase tracking-wider bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded-full">
                  Теорија
                </span>
              )}
              {task.tags?.slice(0, 2).map((tag, i) => (
                <span key={i} className="text-[9px] font-medium uppercase tracking-wider bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full">
                  {tag}
                </span>
              ))}
              {task.tags && task.tags.length > 2 && (
                <span className="text-[9px] font-medium uppercase tracking-wider bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-full">
                  +{task.tags.length - 2}
                </span>
              )}
