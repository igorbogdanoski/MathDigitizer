    ...
    <Card className="bg-white dark:bg-slate-900/60 dark:backdrop-blur-xl border-slate-200 dark:border-white/10 shadow-sm rounded-5xl overflow-hidden">
      <CardContent className="p-8 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              <div className="flex justify-between">
                <label className="text-sm font-bold text-slate-600 dark:text-slate-300">{t('zpd.currentAverage', { value: zpdAvg })}</label>
              </div>
              <input
                type="range" min="0" max="100" value={zpdAvg}
                onChange={(e) => setZpdAvg(parseInt(e.target.value))}
                title={t('zpd.currentAverage', { value: zpdAvg })}
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                <label className="text-sm font-bold text-slate-600 dark:text-slate-300">{t('zpd.momentum', { value: `${zpdVel > 0 ? '+' : ''}${zpdVel}` })}</label>
              </div>
              <input
                type="range" min="-50" max="50" value={zpdVel}
                onChange={(e) => setZpdVel(parseInt(e.target.value))}
                title={t('zpd.momentum', { value: `${zpdVel > 0 ? '+' : ''}${zpdVel}` })}
        ...
        <div className="flex flex-col h-full justify-center">
          <h4 className="text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest mb-2">{t('zpd.targetCapacity')}</h4>
          ...
          <h4 className="text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase tracking-widest mb-2">{t('zpd.nextSteps')}</h4>
