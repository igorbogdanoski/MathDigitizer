    ...
    <Card>
      <CardContent className="p-5 space-y-4">
        <div className="flex items-center gap-2 text-sm font-bold text-slate-700 dark:text-slate-200">
          <Settings2 className="w-4 h-4 text-indigo-500" /> {t('axisSetup.configTitle')}
        </div>
        {(['x', 'y'] as const).map(ax => {
          ...
          return (
            <div key={ax} className="space-y-2 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
              <p className="text-xs font-bold uppercase tracking-widest text-slate-400">{ax === 'x' ? t('axisSetup.horizontal') : t('axisSetup.vertical')}</p>
              <input
                ...
                placeholder={t('axisSetup.axisNamePlaceholder')}
                value={cfg.label}
                onChange={e => setCfg(p => ({ ...p, label: e.target.value }))}
              />
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-xs text-slate-500">{t('axisSetup.min')}</label>
                ...
                <div>
                  <label className="text-xs text-slate-500">{t('axisSetup.max')}</label>
                  <input type="number" className="w-full px-2.5 py-1.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800"
              ...
              <div className="flex gap-2">
                {(['linear', 'log'] as ScaleType[]).map(sc => (
                  <button key={sc} onClick={() => setCfg(p => ({ ...p, scale: sc }))}
                    ...
                    {sc === 'linear' ? t('axisSetup.linear') : t('axisSetup.log')}
                  </button>
                ))}
              </div>
            </div>
        ...
        <Button className="w-full" onClick={onNext}>
          {t('axisSetup.nextCalibration')} <ChevronRight className="w-4 h-4 ml-1" />
        </Button>
      </CardContent>
    </Card>
  );
