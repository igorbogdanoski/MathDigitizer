    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-slate-800 dark:text-slate-200">{t('batch.results', { count: batchTasks.length })}</span>
        </div>
        <Button
          size="sm"
          onClick={onSaveAll}
        ...
        {batchTasks.map((task, i) => (
          <div key={i} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 flex flex-col gap-2">
            <div className="flex items-start justify-between gap-2">
              <span className="font-semibold text-sm text-slate-800 dark:text-slate-200 line-clamp-2 flex-1">{task.title}</span>
              <button
                type="button"
                onClick={() => onCopyTask(task, i)}
                ...
                title={t('batch.copyLatex')}
                aria-label={t('batch.copyLatex')}
              >
                {batchCopied === i ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
              </button>
            </div>
            <div className="flex flex-wrap gap-1.5">
              <span className={`text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${task.difficulty === 'easy' ? 'bg-green-100 text-green-700' : task.difficulty === 'medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>{task.difficulty}</span>
              {task.tags?.slice(0, 2).map((tag, ti) => (
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2">{task.original_text}</p>
          </div>
        ))}
      </div>
    </div>
