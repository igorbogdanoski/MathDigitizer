...
| 8 — GraphDigitizer | ✅ завршена | овој комит |
| 9 — БРО курикулум: комплетност | ⏭️ следна | |
...
9.4 RAG (`curriculumKnowledge`) вклучен во генераторите (денес само класификација).
   Наод од 9.2: `assessment_standards`, `national_standards` и
   `prerequisite_concept_ids` се внесени во корпусот, но ниту еден потрошувач не
   ги чита — се испорачуваат до прелистувачот и никој не ги гледа. Тие се токму
   она што RAG контекстот треба да го носи (по што се оценува исходот, и што
   ученикот мора да знае претходно), па 9.4 ги активира наместо да ги брише.
