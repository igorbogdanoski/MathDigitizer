...
**Immediate action for Phase 0:** lock the Firestore rules gap (pin `role`/`isPro` to unchanged-unless-admin, add `active_user_sessions` rule with teacher-scoping, add ownership checks to `assignments`/`live_sessions`/`whiteboard_sessions`/`task_attempts`), fix the `vite.config.ts` define-block key leak, fix the `ProtectedRoute` null-profile bypass, and harden SSRF protection (metadata IP, alternate IP encodings, redirect-following). These are small, targeted diffs — not a redesign.

**Status: done same-day (2026-07-13), except one item deliberately deferred.** Findings 1–3 and 5–6 above are fixed as of this commit: `firestore.rules` (self-Pro/role-escalation closed, `active_user_sessions`/`assignments`/`live_sessions`/`whiteboard_sessions`/`task_attempts` scoped, syntax-validated against the local Firestore emulator), `vite.config.ts` (leak-causing `define` block removed), `ProtectedRoute.tsx` (null-profile now blocks rather than falling through), and SSRF hardening applied identically to `api/_shared.ts` and `server.ts` (metadata-IP/link-local/IPv6 patterns, decimal/hex/octal host rejection, `redirect: 'manual'` so a 3xx response is never silently followed) plus `server.ts`'s CORS default flipped from fail-open to fail-closed to match `api/_shared.ts`. Verified via `tsc --noEmit`, the full test suite (83/83), `npm run build`, and both quality gates — all green.

**Deliberately deferred: finding 4 (`server.ts`'s unauthenticated `/api/ai/*` proxy).** Properly closing this needs real Firebase ID-token verification, which needs a credentials decision (the only working `firebase-admin` credential pattern already in this repo is a local-machine-only service-account JSON file under `scripts/secrets/`, per `scripts/backup-firestore.mjs` — not something safe to ship to a public-facing server without deliberately deciding how it gets provisioned there). Also worth noting while investigating this: `server.ts` is not actually reachable from the internet in the current production setup — Hostinger serves only the static `dist/` build, and Vercel runs the `api/*.ts` serverless functions instead, which don't include an `/api/ai/*` equivalent at all. So this specific endpoint isn't live-exploitable today, but would become so the moment `server.ts` is ever run somewhere public-facing (e.g. a future dedicated Node host) — worth closing properly before that happens rather than after. Recommend: decide on a credentials-provisioning approach (Vercel environment variable holding a service-account JSON, or switch to Vercel-only hosting for anything that needs a real backend) as its own small follow-up, then add `verifyIdToken` gating to the four `/api/ai/*` routes and thread the ID token through `src/lib/gemini.ts`'s `postJson`.
...
## 10. SEO & SaaS live audit (added same-day, post-Phase-0)

Requested as a live check of the production site while Phase 0 CI ran. Findings, evidence-based:

### SEO

1. **Real bug caught and shipped same-day: an SSRF-hardening regression broke the scraper for nearly every real domain.** The Phase 0 `isSuspiciousNumericHost` check originally compared a hostname's dot-separated label count to a fixed 4, rejecting any domain that wasn't exactly 4 labels — which is most real domains (`algeomath.ru`, `xn--b1agatflbfbtgq5jm.xn--p1ai`, etc. are 2 labels). Caught within minutes via a live bug report on the first real scrape attempt after Phase 0 shipped, fixed and deployed same-day (hotfix PR, see commit history) — now only flags a dotted host when *every* label is itself numeric, which can't false-positive on real domains. Mentioned here rather than in §1 because it was found and fixed entirely within this SEO/SaaS live-audit pass, after Phase 0 was already merged.
2. **`robots.txt` and `sitemap.xml` are healthy** — protected app routes correctly disallowed, sitemap contains exactly the 5 public URLs (`/`, `/pricing`, 3 blog posts), `lastmod` dates auto-update on every deploy.
3. **Structured data (JSON-LD: `SoftwareApplication`, `Service`) is client-side-injected only** (via `react-helmet-async`), confirmed absent from the raw server response and only present after JS execution (verified live via Playwright). This isn't broken — Google's crawler does execute JS — but it's less robust than static/SSR-rendered structured data, and the existing `docs/SAAS_SEO_ACTION_PLAN.md` flagged "test on Google Rich Results" as a manual to-do that was never confirmed done. Recommend actually running https://search.google.com/test/rich-results against the live URL to confirm Google's crawler sees it correctly, rather than assuming.
4. **Observed, self-resolving: Hostinger's `hcdn` edge layer served a stale cached 404 (dated April 2025 — predating this project) for `/library`, `/blog/*`, and other paths on first check, then correctly returned fresh 200s on a retry minutes later** (same pattern as this morning's FTP-deploy staleness mystery in `docs/DEPLOY_TROUBLESHOOTING.md`, but manifesting as a *cached error response* rather than *cached stale content*). By the time this was re-checked, all paths were fresh — likely nudged by the several deploys that happened today. Since the sitemap explicitly asks Google to crawl the 3 blog URLs, and this stale-404 behavior may have been live for a long time before today (unverified — nobody had checked), it's plausible Googlebot hit the stale 404 on a past crawl and marked those URLs as "Not found (404)" in Search Console, which won't self-correct just because the live URL now works — it needs a manual "Request Indexing" resubmission per URL. **Action:** check Search Console's Coverage/Indexing report for these URLs, and if they show a 404-related exclusion, resubmit them. Longer-term, worth asking Hostinger support whether edge-cached error responses have a materially longer effective TTL than the `Cache-Control: max-age=0` the app actually sends.
5. **Minor: inconsistent price-formatting locale on the live pricing page** — "490 МКД" / "4.900 МКД" (dot thousands-separator, Cyrillic label) alongside "4,900 MKD" (comma thousands-separator, Latin label) rendered side by side. Cosmetic, but worth normalizing to one consistent format given the plan's Phase 4 already flags `seo.ts` vs `saas.ts` pricing-source duplication as a drift risk — a good opportunity to fix both at once.

### SaaS

6. **Live pricing currently matches `saas.ts`'s configured values (490/4900 MKD)** — no active drift today, confirmed by rendering the live page. The drift *risk* documented in §4 (hardcoded `seo.ts` structured-data price vs. env-driven `saas.ts` price) remains real for the next time pricing changes, just not currently manifested.
7. No new SaaS-flow bugs found beyond what's already in §1–3 (the Firestore rules fixes in Phase 0 already closed the self-Pro-escalation path and extended the schema to match what the payment-approval flow in `SchoolInquiriesDashboard` actually writes).
